...
def create_mask_from_points(points, img_h, img_w):
    ...
    # Create PIL image and draw filled polygon
...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    # Reshape kernel for conv2d: (out_channels, in_channels, kernel_height, kernel_width)
    laplacian_kernel = laplacian_kernel.view(1, 1, 3, 3)
    
    # Compute Laplacian of foreground image (for each channel)
    laplacian_foreground = torch.zeros_like(foreground_img)
    for c in range(foreground_img.shape[1]):
        laplacian_foreground[:, c:c+1, :, :] = torch.nn.functional.conv2d(
            foreground_img[:, c:c+1, :, :], laplacian_kernel, padding=1
        )
    
    # Compute Laplacian of blended image (for each channel)
    laplacian_blended = torch.zeros_like(blended_img)
    for c in range(blended_img.shape[1]):
        laplacian_blended[:, c:c+1, :, :] = torch.nn.functional.conv2d(
            blended_img[:, c:c+1, :, :], laplacian_kernel, padding=1
        )
    
    # Compute the difference in Laplacian
    laplacian_diff = laplacian_foreground - laplacian_blended
    
    # Apply mask - compute loss only within the mask region
    mask = background_mask.bool()
    mask_expanded = mask.expand(-1, 3, -1, -1)
    
    # Compute mean squared error within the mask region
    loss = torch.mean(laplacian_diff[mask_expanded] ** 2)
